...
func (j jsonrpcDebug) MarshalLogObject(encoder zapcore.ObjectEncoder) error {
	req, err := json.Marshal(j.request)
	if err != nil {
		return err
	}
	...
	encoder.AddString("request", string(req))
	encoder.AddString("response", string(resp))
	return nil
}

// onJsonRpcSend is a callback function used to intercept and log outgoing JSON-RPC messages for debugging purposes.
func onJsonRpcSend(logger *zap.SugaredLogger, req *Request, resp *Response) {
	if logger == nil {
		slog.Info("JSON-RPC", "send", jsonrpcDebug{request: req, response: resp})
		return
	}
	logger.Infow("JSON-RPC", zap.Object("send", jsonrpcDebug{request: req, response: resp}))
}

// onJsonRpcRecv is a callback function used to intercept and log incoming JSON-RPC messages for debugging purposes.
func onJsonRpcRecv(logger *zap.SugaredLogger, req *Request, resp *Response) {
	if logger == nil {
		slog.Info("JSON-RPC", "receive", jsonrpcDebug{request: req, response: resp})
		return
	}
	logger.Infow("JSON-RPC", zap.Object("receive", jsonrpcDebug{request: req, response: resp}))
}
