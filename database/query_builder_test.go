...
func TestUpsertStatement(t *testing.T) {
	tests := []testutils.TestCase[string, testutils.UpsertStatementTestData]{
		...
		{
			Name:     "OverrideTableName_PostgreSQL",
			Expected: `INSERT INTO "custom_table_name" ("email", "id", "name") VALUES (:email, :id, :name) ON CONFLICT ON CONSTRAINT pk_mock_entity DO UPDATE SET "email" = EXCLUDED."email", "id" = EXCLUDED."id", "name" = EXCLUDED."name"`,
			Data: testutils.UpsertStatementTestData{
				Driver:  PostgreSQL,
				Table:   "custom_table_name",
				Columns: []string{"id", "name", "email"},
