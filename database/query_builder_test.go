...
func TestQueryBuilder(t *testing.T) {
	...
	t.Run("PostgreSQL", func(t *testing.T) {
		...
		t.Run("OnConflictConstrainter", func(t *testing.T) {
			...
			qb := &QueryBuilder{subject: &pgsqlConstraintName{}}
			qb.SetColumns("a")
...
func runTests(t *testing.T, db *DB) {
	...
	t.Run("WhereClause", func(t *testing.T) {
		...
		// Is invalid column (1)
		qb := &QueryBuilder{subject: 1}
		assert.PanicsWithValue(t, "qb.Where: unknown subject type provided: \"int\"", func() { _, _ = qb.Where(db, 1) })

		var nilPtr Scoper // Interface nil value
		qb = &QueryBuilder{subject: nilPtr}
		clause, placeholder := qb.Where(db, nilPtr)
		assert.Equal(t, 0, placeholder)
		assert.Empty(t, clause)

		clause, placeholder = qb.Where(db, "id")
		...
		var reference test
		qb = &QueryBuilder{subject: &reference}
		clause, placeholder = qb.Where(db, &reference)
		assertScoperID(clause, placeholder)

		nonNilPtr := new(test)
		qb = &QueryBuilder{subject: nilPtr}
		clause, placeholder = qb.Where(db, nonNilPtr)
		assertScoperID(clause, placeholder)
	})
